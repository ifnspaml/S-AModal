...
# script is slightly different to convert the image-level amodal SAM predictions to video level

pred_data_sam = '/beegfs/work/breitenstein/segment-anything/predictions_asd_predbased_wgt_run2.json'
...
for ann in coco_preds:
    ...
    if ann['track_id'] in unique_tracking_ids:
        ...
        tracking_dict[ann['track_id']]['color'] = [np.random.randint(0, 255), np.random.randint(0, 255),
...
with open('asd_genvisbased_predictions_sam_converted.json', 'w') as f: #'/beegfs/work/breitenstein/sam-pt/predictions_per_video/' +
...
amodal_vis_eval_results = eval_vis(
    test_results='asd_genvisbased_predictions_sam_converted.json', #'/beegfs/work/breitenstein/sam-pt/predictions_per_video/' +
...
pred_coco = coco_gt.loadRes(pred_data_sam)  # Use loadRes to load predictions #'/beegfs/work/breitenstein/sam-pt/predictions_per_video/' +

coco_eval = COCOeval(coco_gt, pred_coco, iouType='segm')#make mistake here so break happens
...
if all_images:
    savepath = '/beegfs/work/breitenstein/sam-pt/eval_asd/asd_genvis_full_visualizations_point_tminus1/'
...
for ann in ytvis_format_sampreds:
    ...
    print('current track id is: ', track_id, flush=True)
    ...
    for k in range(0,len(ann['segmentations'])):  # go through all segmentations
        print('current k is: ', k, flush=True)
        ...
        if ann['segmentations'][k]['counts'] == 'PPYo1' and not_visible_yet:
            ...
            if not cur_occluded:
                cur_occluded = True
                occlusion_start = image_id -1 # was previously image_id because it somehow worked better
                last_appearance = ann['points'][image_id-1]#[1132,  548] #set to -2 (from -1) to have two images where the point actually "appears"

                last_appearance_sanity_check = ann['points'][image_id-1] # need this to get rid of instances that are vanishing
            else:
                cur_occluded = True
            ...
            if last_appearance_sanity_check[0] >= width-20:
                ...
                vanished = True
                continue
            print('current last_appearance is: ', last_appearance, flush=True)
            ...
            previous_mask = mask.decode(ann_new['segmentations'][occlusion_start])#image_id-1])

            # if not all_images:
            ann_new['points'][image_id] = [pred_point[0, -1, 0, 0].item(),pred_point[0, -1, 0, 1].item()]
            ...
            plt.imsave(
                savepath + video + '/' + str(track_id) + '/' + 'shifted_mask_%s_%s.png' % (frames[image_id].split('_rgb')[0], track_id),
                shifted_mask)
...
with open('asd_genvisbased_predictions_sam_converted_occlusions.json', 'w') as f:
...
res_total = {}
amodal_vis_eval_results = eval_vis(
    test_results='asd_genvisbased_predictions_sam_converted_occlusions.json',
    vis_anns=amodal_gt_asd_video,
    maxDets=[1, 10, 100])
eval_results.update(amodal_vis_eval_results)
res_total["amodal per video segm mAP"] = {}

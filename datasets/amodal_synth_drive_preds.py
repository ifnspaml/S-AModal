...
class ASD_Pred_Dataset(data.Dataset):
    """
    dataloader for ASD predictions from GenVis
    """
    ...
    def __init__(self, image_root, gt_root, sam_model, trainsize=352, augmentations=None, train=True, sam_trans=None,test =False,firstcall = False,
        ...
        #additionally we have the path to the predictions:
        if self.test:
            ...
            self.asd_preds = cvb.load(labelspath)
        else:
            labelspath = '/beegfs/work/temp/austausch_franz_jasmin/video_instance_segmentation/AmodalSynthDrive/asd_train_genvis_1080_results.json'
            self.asd_preds = cvb.load(labelspath)
        #need to create IDs for all images in the dataset
        self.image_info = {}

        dict_to_cats = {1: 24, 2: 25, 3: 26, 4: 27, 5: 28, 6: 32, 7: 33}
        if self.test:
            self.coco_gt_dict = {
                        "info": {},
                        "licenses": [],
        ...
        if self.firstcall:
            ...
            #check that we exclude any empty annotations
            count = 0
            ...
            # print(coco_gt_dict_serializable)
...
def get_asd_pred_dataset(sam, sam_trans=None):
    ...
    ds_train = ASD_Pred_Dataset(image_root, gt_root, sam, sam_trans=sam_trans)
    ...
    ds_test = ASD_Pred_Dataset(image_root, gt_root, sam, train=False, sam_trans=sam_trans)
...
if __name__ == '__main__':
    ...
    ds_train, ds_test = get_asd_pred_dataset(sam_trans=sam_trans)
